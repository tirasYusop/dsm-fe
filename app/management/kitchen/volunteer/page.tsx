...
export default function ManagementVolunteersPage() {
  ...
  const handleDelete = async (id: number) => {
    if (!window.confirm("Remove this volunteer from the roster?")) return;
    try {
  ...
  return (
    <RoleGuard allowedRoles={["management"]}>
      <div className="mx-auto space-y-5 p-3">
        <div>
          <h1 className="text-lg font-semibold text-gray-900 sm:text-xl">Volunteer roster</h1>
          <p className="text-sm text-gray-500">
            Register volunteers here — they'll pick their name to clock in/out on the volunteer page.
          </p>
        </div>
          ...
          <Table className="w-full">
            <TableHeader>
              <TableRow className="bg-gray-100">
                <TableHead className="p-2 text-left font-bold">Name</TableHead>
                ...
                <TableRow>
                  <TableCell colSpan={6} className="p-6 text-center text-gray-500">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      No volunteers registered for this kitchen yet.
                    </div>
                  </TableCell>
                </TableRow>
              ) : (
