...
export default function ManagementStudentStoragePage() {
  ...
  const fetchKitchens = async () => {
    try {
      const res = await API.get("/kitchens/"); // adjust to your actual kitchens endpoint if different
  ...
  return (
    <RoleGuard allowedRoles={["management"]}>
      <div className="space-y-5">
        <div>
          <h1 className="text-xl font-semibold text-gray-900">Student Storage</h1>
          <p className="text-sm text-gray-500">
            Raw materials students have logged into kitchen storage. Items over 3 days are flagged.
          </p>
        </div>
        ...
        {alerts.length > 0 && (
          <div className="rounded-lg border border-amber-200 bg-amber-50 p-4">
            <p className="text-sm font-medium text-amber-800">
              {alerts.length} item{alerts.length > 1 ? "s" : ""} expired or expiring today
            </p>
        ...
        <div className="flex flex-wrap items-end gap-4">
          <div>
            <label className="mb-1.5 block text-sm font-medium text-gray-700">Filter by kitchen</label>
            <select
          ...
          <Table className="w-full">
            <TableHeader>
              <TableRow className="bg-gray-100">
                ...
                <TableHead className="p-2 text-left font-bold">Student</TableHead>
                <TableHead className="p-2 text-left font-bold">Kitchen</TableHead>
                <TableHead className="p-2 text-left font-bold">Date Stored</TableHead>
                <TableHead className="p-2 text-left font-bold">Limit</TableHead>
